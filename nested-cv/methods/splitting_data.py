...
def balance_train_smote(features, label, random_state):
    ...
    if num_classes == 2:
        ...
        sampling_strategy_smote = {minority_class: label_counts[max(label_counts, key=label_counts.get)]}
        ...
        minority_classes = {k: v for k, v in label_counts.items() if k != majority_class}
...
def balance_train_adasyn(features, label, random_state):
    ...
    if num_classes == 2:  # Binary case
        ...
        pipeline = Pipeline([
            ("scaler", MinMaxScaler()),
            ("oversample", ADASYN(sampling_strategy=sampling_strategy, random_state=random_state)),
        ])
    else:  # Multiclass case
        majority_class = max(label_counts, key=label_counts.get)
        minority_classes = {k: v for k, v in label_counts.items() if k != majority_class}
