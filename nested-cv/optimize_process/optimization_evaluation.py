...
def optimize_and_save(ref_dirs, dataset_name, algorithm_instance, algorithm_name, model_instance, model_name, problem, train_features, train_classes, val_features, val_classes, filter_info, outer_fold, inner_fold, num_iterations=20):
    ...
    metrics_file = f"outer_fold_{outer_fold}_inner_{inner_fold}_{dataset_name}_metrics_{algorithm_name}_{model_name}.csv"
    evaluation_file = f"outer_fold_{outer_fold}_inner_{inner_fold}_{dataset_name}_evaluation_{algorithm_name}_{model_name}.csv"
    # cross_file = f"outer_fold_{outer_fold}_inner_{inner_fold}_{dataset_name}_cross_validation_{algorithm_name}_{model_name}.csv"
    ...
    with open(fitness_file, "w", newline="") as f, open(solution_file, "w", newline="") as g, open(metrics_file, "w", newline="") as h, open(evaluation_file, "w", newline="") as i:  # , open(cross_file, "w", newline="") as j:
        ...
        for i in range(num_iterations):
            ...
            for population in res.X:
                ...
                # else:
                #    cv_scores = cross_val_score(model_instance, train_features[:, population], train_classes.values, cv=kf, scoring="accuracy")
                #    writerCross.writerow(cv_scores)
