...
from methods.splitting_data import (
    balance_train_smote,
    balance_train_oversample,
    balance_train_undersample,
    balance_train_adasyn,
    balance_train_nearmiss,
    encode_labels,
    balance_train,
...
def optimize_model(
    model_name, dataset_name, algorithm_name, n_outer_folds, n_inner_folds
):
    ...
    for fold_idx, (outer_train_idx, test_idx) in enumerate(
        outer_cv.split(features, classes)
    ):
        ...
        print(
            f"Outer Test Set Dimensions: X_test: {X_test.shape}, y_test: {y_test.shape}"
        )
        ...
        for inner_fold_idx, (inner_train_idx, val_idx) in enumerate(
            ...
            print(
                f"Inner Training Set Dimensions: X_train: {X_train.shape}, y_train: {y_train.shape}"
            )
            print(
                f"Inner Validation Set Dimensions: X_val: {X_val.shape}, y_val: {y_val.shape}"
            )

            # Measure symmetric uncertainty and select features
            symmetric_info_selected = calculate_symmetric_uncertainty(
                X_train, y_train
            )  # ,features_subset
            ...
            problem = FeatureSelectionManyProblem(
