...
def balance_train_oversample(features, label, random_state):
    ...
    if num_classes == 2:  # Binary case
        ...
        pipeline = Pipeline([
                ("scaler", MinMaxScaler()),
                ("oversample",
                 RandomOverSampler(sampling_strategy=sampling_strategy_oversample, random_state=random_state)),
                ("undersample",
                 RandomUnderSampler(sampling_strategy=sampling_strategy_undersample, random_state=random_state)),
            ])
...
def balance_train_undersample(features, label, random_state):
    ...
    if num_classes == 2:  # Binary case
        ...
        pipeline = Pipeline([
                ("scaler", MinMaxScaler()),
                ("oversample", RandomOverSampler(sampling_strategy=sampling_strategy_oversample, random_state=random_state)),
                # ("oversample", ADASYN(sampling_strategy=sampling_strategy_oversample, random_state=random_state)),
                ("undersample", AllKNN(n_neighbors=3)),
                # ("undersample", RandomUnderSampler(sampling_strategy=sampling_strategy_undersample, random_state=random_state)),
                # ("undersample", ClusterCentroids(random_state=random_state)),
                # ("undersample", NearMiss(sampling_strategy=sampling_strategy_undersample, version=1)),
            ])
...
def train_test_val_split(features, classes, balance, encoder, random_state=42):
    X_train, X_test, y_train, y_test = train_test_split(features, classes, stratify=classes, test_size=0.2, random_state=random_state)
