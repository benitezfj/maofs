...
@author: rahmanian
@doi: 10.1109/ICCKE50421.2020.9303633
@link: https://github.com/rahmanian/FMSU/blob/main/fuzzy_utility.py
"""

import numpy as np
import os

def relation_cardinality(sample, M_feature):
        sample = int(sample)
        card = 0
        s = M_feature.shape[1]
        for i in range(s):
...
def fuzzy_entropy(n_samples, M_f):
    h = 0.0
    for i in range(n_samples):
        h -= np.log2(relation_cardinality(i,M_f)/n_samples)
    h = h/n_samples
    return h

def indiscernibility_feature_more(M_f_list):
        min_ = np.minimum.reduce(M_f_list)
        return min_

def indiscernibility_feature(M_f1, M_f2):
        ind = np.minimum(M_f1, M_f2)
        return ind

def ind_cardinality(sample, M_f1, M_f2):
    card = 0
    ind = indiscernibility_feature(M_f1,M_f2)
    card = relation_cardinality(sample, ind)

    return card

def ind_cardinality_more(sample, M_f_list):
    card = 0.0
    ind = indiscernibility_feature_more(M_f_list)
    card = relation_cardinality(sample, ind)

    return card

def joint_fuzzy_entropy(n_samples, M_f1, M_f2):
        h = 0.0
        for i in range(n_samples):
            h -= np.log2(ind_cardinality(i,M_f1,M_f2)/n_samples) 
        h = h / n_samples
        return h

def joint_fuzzy_entropy_more(n_samples,M_f_list):
        h = 0.0
        for i in range(n_samples):
            h -= np.log2(ind_cardinality_more(i,M_f_list)/n_samples) 
        h = h / n_samples
        return h

def fuzzy_mutual_information(M_x, M_y):
        n_samples, n_features = M_x.shape
        Hx = fuzzy_entropy(n_samples,M_x)
        Hy = fuzzy_entropy(n_samples,M_y)
        Hxy = joint_fuzzy_entropy(n_samples,M_x,M_y)
        return Hx + Hy - Hxy

def fuzzy_symmetrical_uncertainty(M_X1, M_X2):
    n_samples, n_features = M_X1.shape
    hx1 = fuzzy_entropy(n_samples, M_X1)
    hx2 = fuzzy_entropy(n_samples, M_X2)
    mi = fuzzy_mutual_information(M_X1, M_X2)
    return 2*mi/(hx1+hx2)

def fuzzy_multivariate_symmetrical_uncertainty(M_f_list):
    n = M_f_list.shape[0]
    sum_ = 0.0
    for M_f in M_f_list:
        sum_ += fuzzy_entropy(M_f)
    hx1_n = joint_fuzzy_entropy_more(M_f_list)
