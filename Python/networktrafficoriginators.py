...
from methods.splitting_data import (
    ...
    balance_train_smote,  # balance_train_nearmiss, balance_train_undersample, balance_train_oversample, balance_train_adasyn, balance_train_smote,
    encode_labels,
)
...
def optimize_model(model_name, dataset_name, algorithm_name):
    ...
    X_train, y_train = balance_train_smote(features=X_train, label=y_train, random_state=42)
    ...
    runner = StarmapParallelization(pool.starmap)
    # runner = StarmapParallelization(pool.starmap, n_processes=n_processes)
